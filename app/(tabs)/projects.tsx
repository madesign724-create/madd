...
export default function ProjectsScreen() {
  ...
  const continueToProjects = async () => {
    await savePendingRoute("/(tabs)/projects");
    ...
    return (
      <ScreenContainer>
        <FlatList
          ...
          ListHeaderComponent={
            <View>
              <AppHeader title="مشاريعي" subtitle="سجّل الدخول لحفظ مشروعك" />
              <EmptyState
                icon="lock-outline"
                title="مشروعك يبدأ بحسابك"
                description="تصفح الخدمات كزائر، ثم سجّل الدخول عندما تريد حفظ مشروع أو اختيار منتج."
                action={<PrimaryButton label="تسجيل الدخول" onPress={() => void continueToProjects()} />}
              />
            </View>
          }
        />
  ...
  return (
    <ScreenContainer>
      <FlatList
        ...
        ListHeaderComponent={
          <View>
            <AppHeader title="مشاريعي" subtitle="تابع التفاصيل مرحلةً بمرحلة" />
            {projects.length > 0 ? (
              <View style={styles.intro}>
                <Text style={styles.kicker}>مساحة العمل الخاصة بك</Text>
                <Text style={styles.introTitle}>كل مشروع، من الرؤية إلى الإنجاز.</Text>
              </View>
            ) : null}
          </View>
        }
        renderItem={({ item }) => {
          const metaText = [item.propertyType, item.city].filter(Boolean).join(" · ") || "تفاصيل المشروع بانتظار الإكمال";
            ...
            >
              <View style={styles.cardTop}>
                ...
                <View style={styles.projectIcon}>
                  <MaterialIcons name="home-work" size={22} color={Brand.pine} />
                </View>
              </View>
              <Text style={styles.projectTitle}>{item.title}</Text>
              <Text style={styles.projectMeta}>{metaText}</Text>
              <View style={styles.cardRule} />
              <View style={styles.cardFooter}>
                <Text style={styles.openText}>فتح مساحة المشروع</Text>
                <MaterialIcons name="arrow-back" size={16} color={Brand.pine} />
              </View>
            </Pressable>
          );
        }}
        ListEmptyComponent={
          <EmptyState
            ...
            description="أنشئ مسودة بسيطة، ثم اختر الخدمات والمنتجات التي تناسبك."
            action={
              <PrimaryButton
                label="إنشاء مشروع"
