...
export function AdminCatalogHierarchy() {
  ...
  const deleteService = trpc.admin.deleteService.useMutation({
    onSuccess: (result) => {
      ...
      Alert.alert(
        "تم الحذف",
        result.linkedProjectCount
          ? `حُذفت الصفحة الرئيسية من الكتالوج مع بقاء سجلها محفوظاً في ${result.linkedProjectCount} مشروع.`
          : "حُذفت الصفحة الرئيسية ومحتواها من الكتالوج."
      );
    },
  ...
  const deleteNode = trpc.admin.deleteCatalogNode.useMutation({
    onSuccess: (result) => {
      ...
      Alert.alert(
        "تم الحذف",
        result.linkedProjectCount
          ? `حُذفت التقسيمة من الكتالوج مع بقاء سجلها محفوظاً في ${result.linkedProjectCount} مشروع.`
          : "حُذفت التقسيمة وكل محتواها من الكتالوج."
      );
    },
    onError: (error) => Alert.alert("تعذر الحذف", error.message),
  });
  ...
  const sortedProducts = useMemo(
    () =>
      [...products].sort(
        (left, right) =>
          (left.catalogNodeId ?? 0) - (right.catalogNodeId ?? 0) ||
          left.sortOrder - right.sortOrder ||
          left.name.localeCompare(right.name, "ar") ||
          left.id - right.id
      ),
  ...
  const collapseAll = () => {
    const collapsibleNodes = sortedNodes
      .filter(
        (node) =>
          sortedNodes.some((candidate) => candidate.parentId === node.id) ||
          sortedProducts.some((product) => product.catalogNodeId === node.id)
      )
      .map((node) => `node:${node.id}`);
    const collapsibleServices = services
      .filter(
        (service) =>
          sortedNodes.some((node) => node.serviceId === service.id && node.parentId === null) ||
          sortedProducts.some((product) => product.serviceId === service.id && product.catalogNodeId === null)
      )
  ...
  const confirmDelete = () => {
    ...
    if (deleteTarget.targetType === "service") {
      deleteService.mutate({
        serviceId: deleteTarget.targetId,
        confirmLinkedDeletion: deleteTarget.requiresLinkedDeletionConfirmation,
      });
    } else {
      deleteNode.mutate({
        nodeId: deleteTarget.targetId,
        confirmLinkedDeletion: deleteTarget.requiresLinkedDeletionConfirmation,
      });
    }
  };
  ...
  const renderClipboardActions = (sourceType: ClipboardItem["sourceType"], sourceId: number, label: string) => (
    <View style={styles.actions}>
      <Pressable
        accessibilityLabel={`نسخ ${label}`}
        onPress={() => setClipboardItem({ action: "copy", sourceType, sourceId, label })}
        style={({ pressed }) => [
          styles.actionButton,
          clipboard?.action === "copy" && clipboard.sourceType === sourceType && clipboard.sourceId === sourceId && styles.actionActive,
      ...
      <Pressable
        accessibilityLabel={`قص ${label}`}
        onPress={() => setClipboardItem({ action: "cut", sourceType, sourceId, label })}
        style={({ pressed }) => [
          styles.actionButton,
          clipboard?.action === "cut" && clipboard.sourceType === sourceType && clipboard.sourceId === sourceId && styles.actionCutActive,
          pressed && styles.pressed,
        ]}
      >
        <MaterialIcons name="content-cut" size={15} color={Brand.ink} />
        <Text style={styles.actionText}>قص</Text>
      </Pressable>
      <Pressable
        ...
        onPress={() =>
          requestDelete({
            ...
            linkedSelectionCount: 0,
            requiresLinkedDeletionConfirmation: false,
          })
        }
        style={({ pressed }) => [styles.actionButton, styles.deleteButton, pressed && styles.pressed]}
      >
        <MaterialIcons name="delete-outline" size={15} color={Brand.error} />
  ...
  const renderPasteAction = (targetType: PasteTarget["targetType"], targetId: number, label: string) => {
    ...
    return (
      <Pressable
        accessibilityLabel={`لصق داخل ${label}`}
        onPress={() => requestPaste({ targetType, targetId, label })}
        style={({ pressed }) => [styles.pasteButton, pressed && styles.pressed]}
      >
        <MaterialIcons name="content-paste" size={14} color="#0C0C0C" />
        <Text style={styles.pasteText}>لصق هنا</Text>
      </Pressable>
    );
  };
  ...
  const renderTreeNode = (node: CatalogNode, depth: number): ReactNode => {
    ...
    return (
      <View key={node.id} style={[styles.branch, { marginRight: depth * 14 }]}>
        <View style={styles.nodeRow}>
          <View style={styles.nodeIcon}>
            <MaterialIcons name={node.nodeType === "category" ? "account-tree" : "tune"} size={17} color={Brand.pine} />
          </View>
          <View style={styles.nodeCopy}>
            <Text style={styles.nodeName}>{node.name}</Text>
            <Text style={styles.nodeMeta}>
              {node.nodeType === "category" ? "تقسيمة" : "خيار"} · {selectionModeLabel[node.selectionMode]} · {node.isActive ? "منشور" : "مخفي"}
            </Text>
          </View>
          {hasChildren ? (
            <Pressable
              accessibilityLabel={isCollapsed ? `فتح فرع ${node.name}` : `طي فرع ${node.name}`}
              onPress={() => toggleBranch(branchKey)}
              style={({ pressed }) => [styles.toggleButton, pressed && styles.pressed]}
            >
              <MaterialIcons name={isCollapsed ? "chevron-left" : "expand-more"} size={20} color={Brand.pine} />
            </Pressable>
          ) : null}
        </View>
        <View style={styles.nodeTools}>
          {renderClipboardActions("node", node.id, nodeLabel)}
          {renderPasteAction("node", node.id, nodeLabel)}
        </View>
        {!isCollapsed ? (
          <View>
            {children.map((child) => renderTreeNode(child, depth + 1))}
            {nodeProducts.map((product) => (
              <View key={product.id} style={[styles.productRow, { marginRight: 14 }]}>
                <MaterialIcons name="inventory-2" size={15} color={Brand.muted} />
  ...
  const renderServiceBranch = ({ item: service }: { item: typeof services[number] }) => {
    ...
    const directProducts = sortedProducts.filter(
      (product) => product.serviceId === service.id && product.catalogNodeId === null
    );
    return (
      <View style={styles.serviceBranch}>
        <View style={styles.serviceHeader}>
          <Text style={styles.serviceName}>{service.name}</Text>
          <Pressable
            accessibilityLabel={isServiceCollapsed ? `فتح الصفحة الرئيسية ${service.name}` : `طي الصفحة الرئيسية ${service.name}`}
            onPress={() => toggleBranch(serviceKey)}
            style={({ pressed }) => [styles.toggleButton, pressed && styles.pressed]}
          >
            <MaterialIcons name={isServiceCollapsed ? "chevron-left" : "expand-more"} size={20} color={Brand.pine} />
          </Pressable>
        </View>
        <View style={styles.serviceTools}>
          {renderClipboardActions("service", service.id, serviceLabel)}
          {renderPasteAction("service", service.id, serviceLabel)}
        </View>
        {!isServiceCollapsed ? (
          <View>
            ...
            {directProducts.map((product) => (
              <View key={product.id} style={styles.productRow}>
                ...
                <View style={styles.productCopy}>
                  ...
                  <Text style={styles.productMeta}>منتج مباشر داخل الصفحة</Text>
                </View>
              </View>
            ))}
          </View>
  ...
  return (
    <View style={styles.launcher}>
      <Pressable
        accessibilityLabel="عرض هيكل الكتالوج وحافظة النسخ والقص واللصق"
        onPress={() => setVisible(true)}
        style={({ pressed }) => [styles.launcherButton, pressed && styles.pressed]}
      >
        <View style={styles.launcherCopy}>
          <Text style={styles.launcherTitle}>هيكل الكتالوج وحافظة المحتوى</Text>
          <Text style={styles.launcherSubtitle}>انسخ أو انقل فرعاً كاملاً ثم ألصقه في مكانه الصحيح</Text>
        </View>
      ...
      <BottomSheet visible={visible} onRequestClose={close}>
        <View style={styles.sheet}>
          <View style={styles.sheetTop}>
            <Pressable accessibilityLabel="إغلاق" onPress={close} style={({ pressed }) => [styles.close, pressed && styles.pressed]}>
              <MaterialIcons name="close" size={21} color={Brand.ink} />
            </Pressable>
            <Text style={styles.sheetTitle}>هيكل الكتالوج</Text>
          </View>
            ...
            ListHeaderComponent={
              <View>
                <View style={styles.intro}>
                  <MaterialIcons name="format-list-numbered-rtl" size={22} color={Brand.pine} />
                  <View style={styles.introCopy}>
                    <Text style={styles.heading}>شجرة الكتالوج والحافظة</Text>
                    <Text style={styles.help}>
                      اضغط «نسخ» لإنشاء نسخة مستقلة، أو «قص» لنقل الفرع. بعدها اضغط «لصق هنا» في الصفحة أو التقسيمة الصحيحة. يشمل الإجراء كل الفروع والمنتجات والصور وترتيبها.
                    </Text>
                  </View>
                </View>

                <View style={styles.treeControls}>
                  <Pressable accessibilityLabel="فتح كل الفروع" onPress={expandAll} style={({ pressed }) => [styles.treeControl, pressed && styles.pressed]}>
                    <MaterialIcons name="unfold-more" size={16} color={Brand.ink} />
                    <Text style={styles.treeControlText}>فتح الكل</Text>
                  </Pressable>
                  <Pressable accessibilityLabel="طي كل الفروع" onPress={collapseAll} style={({ pressed }) => [styles.treeControl, pressed && styles.pressed]}>
                    <MaterialIcons name="unfold-less" size={16} color={Brand.ink} />
                    <Text style={styles.treeControlText}>طي الكل</Text>
                  </Pressable>
                </View>

                {clipboard ? (
                  <View style={styles.clipboardCard}>
                    <MaterialIcons name={clipboard.action === "copy" ? "content-copy" : "content-cut"} size={20} color={Brand.pine} />
                    <View style={styles.clipboardCopy}>
                      <Text style={styles.clipboardTitle}>
                        {clipboard.action === "copy" ? "جاهز للنسخ" : "جاهز للقص والنقل"}: {clipboard.label}
                      </Text>
                      <Text style={styles.clipboardHelp}>
                        {clipboard.action === "copy"
                          ? "يمكنك اللصق في أكثر من مكان. الأصل لن يتغير."
                          : "اختر مكاناً جديداً ثم ألصق؛ سيُنقل الفرع كاملاً من موضعه الحالي."}
                      </Text>
                    </View>
                    <Pressable
                      accessibilityLabel="إفراغ الحافظة"
                      onPress={() => {
                        setClipboard(null);
                        setPasteTarget(null);
                        setConfirmingPaste(false);
                      }}
                      style={({ pressed }) => [styles.clearClipboard, pressed && styles.pressed]}
                    >
                      <MaterialIcons name="close" size={17} color={Brand.ink} />
                    </Pressable>
                  </View>
                ) : null}

                {nodesQuery.isLoading || productsQuery.isLoading || servicesQuery.isLoading ? (
                  <ActivityIndicator color={Brand.pine} style={styles.loader} />
                ) : null}

                {!nodesQuery.isLoading && !productsQuery.isLoading && !servicesQuery.isLoading && services.length === 0 ? (
                  <Text style={styles.help}>لا توجد صفحات رئيسية في الكتالوج حالياً.</Text>
                ) : null}
              </View>
            }
          />

          {confirmingPaste && clipboard && pasteTarget ? (
            <View style={styles.pasteConfirmOverlay}>
              <Pressable
                accessibilityLabel="إلغاء تأكيد اللصق"
                onPress={() => {
                  if (!pasteClipboard.isPending) {
                    setConfirmingPaste(false);
                    setPasteTarget(null);
                  }
                }}
              ...
              <View accessibilityViewIsModal style={styles.confirmBox}>
                <Text style={styles.confirmTitle}>
                  تأكيد {clipboard.action === "copy" ? "النسخ" : "القص واللصق"}
                </Text>
                <Text style={styles.confirmText}>
                  {clipboard.action === "copy" ? "سيُنشئ النظام نسخة مستقلة" : "سينقل النظام الشجرة الأصلية"} من «
                  {clipboard.label}» إلى داخل «{pasteTarget.label}».{" "}
                  {clipboard.action === "copy"
                    ? "سيبقى المصدر بلا أي تغيير."
                    : "ستنتقل الفروع والمنتجات والصور وترتيبها إلى المكان الجديد."}
                </Text>
                {pasteClipboard.error ? <Text style={styles.errorText}>{pasteClipboard.error.message}</Text> : null}
                <View style={styles.confirmActions}>
                  <SecondaryButton
                    label="إلغاء"
                    disabled={pasteClipboard.isPending}
                    onPress={() => {
                      setConfirmingPaste(false);
                      setPasteTarget(null);
                    }}
                  ...
                  />
                  <PrimaryButton
                    label={clipboard.action === "copy" ? "تأكيد النسخ" : "تأكيد النقل"}
                    loading={pasteClipboard.isPending}
      ...
      {deleteTarget ? (
        <DeleteConfirmationDialog
          ...
          confirmLabel={
            deleteTarget.requiresLinkedDeletionConfirmation
              ? "حذف مع حفظ السجل"
              : deleteTarget.targetType === "service"
              ? "حذف الصفحة"
              : "حذف التقسيمة"
...
const styles = StyleSheet.create({
  ...
  sheet: {
    flex: 1,
    height: Math.floor(Dimensions.get("window").height * 0.88),
  ...
  treeList: { flex: 1, width: "100%" },
  content: { padding: 16, paddingBottom: 120 }, // مساحة كافية لتمرير كل العناصر وتجاوز شريط الأزرار
