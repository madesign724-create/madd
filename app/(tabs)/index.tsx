      ...
      <FlatList
        ...
        ListHeaderComponent={
          <>
            ...
            <View style={styles.hero}>
              ...
              {company?.logoUrl ? (
                <Image source={{ uri: resolveAssetUrl(company.logoUrl) }} style={styles.heroLogo} />
              ) : (
                <View style={styles.heroMonogram}><Text style={styles.heroMonogramText}>M</Text></View>
              )}
            </View>
            ...
            <View style={styles.sectionHeading}>
              ...
              <Pressable onPress={() => router.push("/(tabs)/services" as never)} style={({ pressed }) => [styles.allLink, pressed && styles.pressed]}>
                <Text style={styles.allLinkText}>عرض الكل</Text>
                <MaterialIcons name="arrow-back" size={15} color={Brand.pine} />
              </Pressable>
            </View>
          </>
        }
        renderItem={({ item }) => (
          <Pressable onPress={() => router.push(`/service/${item.id}` as never)} style={({ pressed }) => [styles.serviceCard, pressed && styles.pressed]}>
            {item.imageUrl ? (
              <Image source={{ uri: resolveAssetUrl(item.imageUrl) }} style={styles.serviceImage} />
            ) : (
              <View style={styles.serviceImageFallback}><MaterialIcons name="dashboard-customize" size={28} color={Brand.pine} /></View>
            )}
            ...
            <View style={styles.serviceCopy}>
              ...
              <View style={styles.serviceArrow}>
                <MaterialIcons name="arrow-back" size={16} color="#0C0C0C" />
              </View>
            </View>
          </Pressable>
        )}
        ListEmptyComponent={
          !servicesQuery.isLoading ? (
            <View style={styles.notice}>
              <MaterialIcons name="inventory-2" size={23} color={Brand.pine} />
              <Text style={styles.noticeText}>سيظهر كتالوج الخدمات هنا فور إضافة فريق MADD للخدمات والصور من لوحة الإدارة.</Text>
            </View>
          ) : null
        }
      />
...
const styles = StyleSheet.create({
  ...
  topBar: { minHeight: 54, flexDirection: "row-reverse", alignItems: "center", marginBottom: 18 },
  ...
  companyBlock: { flex: 1, alignItems: "flex-end", marginRight: 11 },
  ...
  notificationBadge: { position: "absolute", top: -4, left: -4, minWidth: 17, height: 17, paddingHorizontal: 4, borderRadius: 9, backgroundColor: Brand.pine, alignItems: "center", justifyContent: "center", borderWidth: 1, borderColor: Brand.canvas },
  ...
  heroGlow: { width: 190, height: 190, borderRadius: 95, backgroundColor: "#3B2F12", opacity: 0.72, position: "absolute", right: -78, top: -60 },
  heroContent: { flex: 1, alignSelf: "stretch", alignItems: "flex-end", justifyContent: "flex-end", zIndex: 1 },
  ...
  heroTitle: { alignSelf: "flex-end", color: Brand.ink, fontSize: 30, lineHeight: 39, fontWeight: "900", textAlign: "right", writingDirection: "rtl", marginTop: 11, width: "88%" },
  heroText: { alignSelf: "flex-end", color: Brand.muted, fontSize: 13, lineHeight: 21, textAlign: "right", writingDirection: "rtl", marginTop: 11, width: "76%" },
  heroRule: { width: 44, height: 2, backgroundColor: Brand.pine, borderRadius: 2, marginTop: 17, alignSelf: "flex-end" },
  ...
  sectionHeading: { flexDirection: "row-reverse", justifyContent: "space-between", alignItems: "flex-end", marginTop: 32, marginBottom: 14 },
  ...
  allLink: { minHeight: 34, flexDirection: "row-reverse", alignItems: "center", gap: 6 },
  ...
  serviceCopy: { flex: 1, flexDirection: "row-reverse", alignItems: "flex-end", justifyContent: "space-between", padding: 16 },
  serviceArrow: { width: 34, height: 34, borderRadius: 17, alignItems: "center", justifyContent: "center", backgroundColor: Brand.pine },
  serviceText: { flex: 1, alignItems: "flex-end", marginLeft: 12 },
